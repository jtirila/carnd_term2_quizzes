	...
	*****************************************************************************/
    ...
    std::cout << "About to print landmark info" << "\n";
		while(getline(in_file_map, line_map)){
			...
			std::cout << landmark_x_f << std::endl;
	...
	std::string line;
	std::cout << filename_control << std::endl;
	...
	while(getline(in_file_control, line)){
		...
		std::cout << sprintf(str_obs,"%sobservations_%06i.txt", filename_obs.c_str(), count) << std::endl;
		std::cout << std::string(str_obs) << std::endl;
