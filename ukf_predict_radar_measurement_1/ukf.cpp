 ...
 ******************************************************************************/
  ...
  for(int col = 0; col < Xsig_pred.cols(); col++ ){
    ...
    v = Xsig_pred(2, col);
    phi = Xsig_pred(3, col);
    phidot = Xsig_pred(4, col);

    rho = std::sqrt(px * px + py * py);
    phi_meas = std::atan(py / px);
    if(phi_meas < 0 && py  > 0)
      phi_meas += M_PI;
    if(phi_meas > 0 && py < 0)
      phi_meas -= M_PI;
  ...
  for(int col = 0; col < Zsig.cols(); col++){
    ...
    while(diff(1) > M_PI)
      diff(1) -= 2 * M_PI;
    S += weights(col) * diff * diff.transpose();
  ...
  MatrixXd noise(3, 3);
  noise.fill(0.0);
  noise(0, 0) = std_radr * std_radr;
  noise(1, 1) = std_radphi * std_radphi;
  noise(2, 2) = std_radrd * std_radrd;
